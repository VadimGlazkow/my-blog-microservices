document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-post-id]').forEach(element => {
    ...
    socket.onclose = (event) => console.error(`WebSocket closed for post ${postId}:`, event);
  ...
  document.querySelectorAll('.like-btn, .dislike-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      if (!button.classList.contains('disabled')) {
        ...
        button.classList.add('disabled'); // Блокировка кнопки
        fetch(`/post/${postId}/rate/`, {
          ...
          .finally(() => button.classList.remove('disabled')); // Разблокировка
      }
    });
  });
  ...
  function updateRatingButtons(postId, data) {
    ...
    document.querySelectorAll(`[data-post-id="${postId}"]`).forEach(button => {
      ...
      // Обновляем счётчики для всех пользователей
      if (isLikeBtn) {
      ...
      // Обновляем состояние кнопки только для авторизованных пользователей
      if (isAuthenticated) {
        if (isLikeBtn) {
        ...
        // Для неавторизованных пользователей сбрасываем состояние
